...
const BusinessNewsList = () => {
  ...
  const pageCount = businessNews ? Math.ceil(businessNews.length / pageSize)
      : 0;
  ...
  const pagination = (pageNo) => {
    ...
    const paginatedBusinessNew = _(businessNews).slice(startIndex).take(
        pageSize).value();
  ...
  return (
      <div className="container">
        ...
        <hr/>
        <div>
          <Link to="/createNews" className="btn btn-primary mb-2">Add Business
            News</Link>
                  ...
                  <tr key={b.id}>
                    ...
                    <td>
                      <Link className="btn btn-info"
                            to={`/updateNews/${b.id}`}>Update</Link>
