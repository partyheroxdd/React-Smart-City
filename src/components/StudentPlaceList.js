...
const StudentPlaceList = () => {
  ...
  const pageCount = studentPlaces ? Math.ceil(studentPlaces.length / pageSize)
      : 0;
  ...
  const pagination = (pageNo) => {
    ...
    const paginatedStudentPlaces = _(studentPlaces).slice(startIndex).take(
        pageSize).value();
  ...
  return (
      <div className="container">
        ...
        <hr/>
        <div>
          <Link to="/createStudentPlace" className="btn btn-primary mb-2">Add
            Student Place</Link>
                  ...
                  <tr key={s.id}>
                    ...
                    <td>
                      <Link className="btn btn-info"
                            to={`/updateStudentPlace/${s.id}`}>Update</Link>
