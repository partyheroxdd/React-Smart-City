...
const TouristicPlaceList = () => {
  ...
  const pageCount = touristcPlaces ? Math.ceil(touristcPlaces.length / pageSize)
      : 0;
  ...
  const pagination = (pageNo) => {
    ...
    const paginatedStudentPlaces = _(touristcPlaces).slice(startIndex).take(
        pageSize).value();
  ...
  return (
      <div className="container">
        ...
        <hr/>
        <div>
          <Link to="/createTouristicPlace" className="btn btn-primary mb-2">Add
            Touristic Place</Link>
                  ...
                  <tr key={t.id}>
                    ...
                    <td>
                      <Link className="btn btn-info"
                            to={`/updateTouristicPlace/${t.id}`}>Update</Link>
